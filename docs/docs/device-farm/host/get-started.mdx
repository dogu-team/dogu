...
This page covers how to start the [`Dost`](/device-farm/introduction#host-and-dost) program, which is installed on the [`Host`](/device-farm/introduction#host-and-dost) machine.
...
Please go to the [Managing Hosts](/management/organization/host) page to obtain the host token and enter the obtained token in Dost.
