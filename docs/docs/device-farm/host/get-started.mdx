...
This page covers how to start the [`Dogu Agent`](/device-farm#host-and-agent) program, which is installed on the [`Host`](/device-farm#host-and-agent) machine.
...
You can download `Dogu Agent` from the [Dogu Agent Download](https://dogutech.io/downloads/dost) page.
...
Please go to the [Managing Hosts](/management/organization/host) page to obtain the host token and enter the obtained token in Dogu Agent.
...
Dogu Agent installation has been completed? Try adding devices through [Device configuration](../device/settings).
