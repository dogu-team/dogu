...
You can set up a test CI/CD environment by integrating Routine and Github Action.
...
#### 4. Enter "DOGU_PROJECT_TOKEN" in `Name`, paste the copied Token into `Secret`, and then click the `Add secret` button.
...
- If the Simple Workflow does not appear, search for "Simple workflow" and select it.
