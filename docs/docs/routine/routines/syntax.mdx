...
Apps specified as `Latest` can be used in routines without specifying a version.  
To upload the app to the latest version, refer to the [App Management](/management/project/app) documentation.

:::

1. If you want to run the app specified as `Latest` on all platforms of the target device:
   > Example
   >
   > ```yaml
   > jobs:
   ...
   >     appVersion: latest
   > ```
2. If you want to run the app specified as `Latest` on each platform of the target device:
   > Example
   >
   > ```yaml
   > jobs:
   ...
   >       ios: latest
   > ```
3. If you want to use a single version for all platforms of the target device:
   > Example
   >
   > ```yaml
   > jobs:
   ...
   >     appVersion: 1.0.0
   > ```
4. If you want to use different versions for each platform of the target device:
   > Example
   >
   > ```yaml
   > jobs:
