...
Therefore, after executing the `Install app and run` action, the `Run test` action must be executed for it to work. For more information on actions, see the [Actions](./actions) document.
:::
