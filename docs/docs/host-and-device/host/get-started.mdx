...
| macOS   | macOS 10.13 (High Sierra) or later | x64, arm64   |
...
- Start Dost when ayou log in
  - Dost starts automatically when PC turns on.
- Theme for Dost
  - Sets Dost's color theme.
...
- WebDriverAgent
  ...
  - Afterwards, if the warning message or error message disappears, return to Dost and click the `Click here to build` button..
...
- iOSDeviceAgent
  ...
  - Go back to Dost and hit the 'Click here to build' button.
  - When the build is complete, press the ‘Check’ button to make a green check mark appear..
