...
1. Dogu의 프로젝트 페이지로 이동합니다.
2. 프로젝트 이름 옆에 `Git` 아이콘이 초록색으로 표시되는지 확인합니다.
   <img
     src="/ko/img/console/project/git-tag.png"
     style={{ width: 500, display: 'block' }}
   />
3. 루틴 탭을 선택하고 `+` 아이콘을 클릭합니다.
4. GUI 에디터를 선택하고, `Run test` 액션에서 테스트 스크립트가 불러와지는지 확인합니다.
   <img
