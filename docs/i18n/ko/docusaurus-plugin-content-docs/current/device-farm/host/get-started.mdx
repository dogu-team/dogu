...
이 페이지에서는 [`호스트`](/device-farm#host-and-agent)에 설치되는 프로그램인 [`Dogu Agent`](/device-farm#host-and-agent)를 시작하는 방법에 대해 다루고 있습니다.
...
[Dogu Agent 다운로드](https://dogutech.io/downloads/dost) 페이지에서 `Dogu Agent`를 다운로드 받을 수 있습니다.  
...
[호스트 관리](/management/organization/host) 페이지를 확인하여 호스트 토큰을 획득하고, 획득한 토큰을 Dogu Agent에 입력해주세요.
...
- dmg file을 열어 Dogu Agent를 Applications 폴더로 이동해주세요.
...
Dogu Agent 추가를 완료하셨나요? [디바이스 설정하기](../device/settings) 를 통해 디바이스들을 추가해보세요.
