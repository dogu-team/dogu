...
2. `adb devices` 명령어를 입력하세요.
   ...
   - `offline`: 디바이스에서 호스트 PC에 연결할 때 디바이스 허용을 선택하거나, 디바이스에서 개발자 옵션을 활설화 한 후, `USB 디버깅` 옵션을 활성화합니다.
...
3. 위 방법으로도 연결되지 않는다면 Android 기기를 재시작해보세요.
...
2. Dogu Agent에서 iOS Settings 항목이 전부 완료되었는지 확인해주세요.
...
3. Dogu Agent에서 `View Devices and Simulators` 를 클릭하여 Xcode 내에 Devices 창을 실행해주세요.
...
4. iOS기기에 Developer가 Untrusted인 상태인지 확인해주세요.
...
5. 위 방법으로도 연결되지 않는다면 iOS기기를 재시작해보세요.
...
Dogu Agent가 최신 버전인지 확인해주세요. Dogu Agent 최신 버전은 [여기](https://dogutech.io/downloads/dost)에서 다운로드 받을 수 있습니다.
