...
( 위에서 설정할 ip는 [docker-compose.env 파일 생성](/get-started/installation/self-hosted/installation/#docker-composeenv-파일-생성)에서 설정한 DOGU_HOST값과 동일한 값입니다. )
...
[github releases](https://github.com/dogu-team/dogu/releases?q=dost&expanded=true)에서 Dogu Agent를 다운로드 해주세요.

다운로드 후 설치는 [Agent 설치하기](/device-farm/host#설치)를 참고해주세요.

### Dogu Agent 실행

dogu console에 연결할 수 있는 Dogu Agent를 실행합니다.

- Dogu Agent 접속 환경 설정

  - [docker-compose.env 파일 생성](/get-started/installation/self-hosted/installation#docker-composeenv-파일-생성)에서 설정한 DOGU_HOST값을 사용하여 접속 URL을 설정합니다.
    :::note
...
구성하고자 하는 테스트 환경에 맞게 디바이스 팜을 구성합니다. 구성 방법은 [디바이스 팜 구축](/get-started/tutorials/device-farm) 문서를 참고해 주세요.
