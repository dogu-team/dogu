...
( 위에서 설정할 ip는 [docker-compose.env 파일 생성](/self-hosted/get-started#docker-composeenv-파일-생성)에서 설정한 DOGU_HOST값과 동일한 값입니다. )
...
:::note
GITLAB_ROOT_ACCESS_TOKEN 값은 Dogu 실행 후 아래 [gitlab root Access Token 발급 받기](/self-hosted/get-started#gitlab-root-access-token-발급-받기)에서 token을 발급받은 후 설정 해 주세요.
:::
...
다운로드 후 설치는 [Dost 설치](/device-farm/host/get-started#설치)를 참고해주세요.
...
[Tutorial - Device farm](/get-started/tutorials/device-farm) 문서에서 확인할 수 있습니다.
