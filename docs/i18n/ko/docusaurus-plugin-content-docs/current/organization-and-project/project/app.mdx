---
sidebar_position: 5
title: 앱 관리
locale: ko
---

import ProjectWritePermission from '/src/components/console/ProjectWritePermission.mdx';
import ProjectMemberPermission from '/src/components/console/ProjectMemberPermission.mdx';

프로젝트에서 Gamium Engine SDK가 적용된 앱을 업로드하고, 루틴이나 스크립트 테스트에 활용할 수 있습니다.

:::info

게임에 Gamium Engine SDK 적용하는 방법은 [Gamium](https://gamium.dogutech.io) 문서를 참고하세요.

:::

## 앱 정보

Dogu는 다음과 같은 정보를 앱에서 수집합니다.

- 앱 ID
- 앱 이름
- 앱 버전
- 앱 아이콘
- 앱 크기

## 앱 업로드하기

:::info 지원 확장자

앱 업로드는 `.apk` 파일만 지원합니다.

:::

<ProjectWritePermission prefix="앱 업로드는" />

1. 프로젝트 페이지에서 "앱" 탭을 클릭합니다.
2. "앱 업로드" 버튼을 클릭합니다.
   <img
     src="/ko/img/console/project/upload-app.png"
     style={{ width: 600, display: 'block' }}
   />
3. 업로드할 앱을 드래그하거나 선택해 업로드를 완료합니다.
   <img
     src="/ko/img/console/project/upload-app-modal.png"
     style={{ width: 600, display: 'block' }}
   />

:::caution 앱 버전

앱 버전이 같은 앱이 있으면 앱을 덮어 씌웁니다.

:::

## 앱 활용하기

### 루틴

루틴을 통한 게임 테스트를 진행할 때, 업로드 된 앱 버전을 활용합니다. 해당 앱 버전의 앱이 목록에 없을 경우, 루틴은 작동하지 않습니다.

루틴에서 앱 버전을 사용하는 방법은 [루틴 가이드](/script-and-routine/routine/about)를 참고하세요.

### 스크립트 테스트

스크립트 테스트를 진행할 때, 업로드 된 앱 버전을 활용합니다. 앱 선택상자에서 앱 버전을 선택해 스크립트 테스트를 진행할 수 있습니다.

스크립트 테스트에 대한 자세한 내용은 [스크립트 테스트](./script-test)를 참고하세요.

## 앱 다운로드

<ProjectMemberPermission prefix="앱 다운로드는" />

1. 프로젝트 페이지에서 "앱" 탭을 클릭합니다.
2. 삭제할 앱을 목록에서 찾고 메뉴 버튼(`···`)을 클릭합니다.
3. "다운로드" 버튼을 클릭합니다.
   <img
     src="/ko/img/console/project/download-app.png"
     style={{ width: 600, display: 'block' }}
   />

## 앱 삭제하기

<ProjectWritePermission prefix="앱 삭제는" />

1. 프로젝트 페이지에서 "앱" 탭을 클릭합니다.
2. 삭제할 앱을 목록에서 찾고 메뉴 버튼(`···`)을 클릭합니다.
3. "삭제" 버튼을 클릭합니다.
   <img
     src="/ko/img/console/project/delete-app.png"
     style={{ width: 600, display: 'block' }}
   />
