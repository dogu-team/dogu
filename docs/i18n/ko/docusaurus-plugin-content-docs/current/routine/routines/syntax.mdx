...
`Latest`로 지정된 앱은 루틴에서 버전 지정 없이 사용할 수 있습니다.  
앱을 최신 버전으로 업로드 하는 방법은 [앱 관리](/management/project/app) 문서를 참고하세요.

:::

1. 타겟 디바이스의 모든 플랫폼에서 `Latest`로 지정된 앱을 실행하는 경우
   > 예시
   >
   > ```yaml
   > jobs:
   ...
   >     appVersion: latest
   > ```
2. 타겟 디바이스의 각 플랫폼에서 `Latest`로 지정된 앱을 실행하는 경우
   > 예시
   >
   > ```yaml
   > jobs:
   ...
   >       ios: latest
   > ```
3. 타겟 디바이스의 모든 플랫폼에서 하나의 버전을 사용할 경우
   > 예시
   >
   > ```yaml
   > jobs:
   ...
   >     appVersion: 1.0.0
   > ```
4. 타겟 디바이스의 각 플랫폼에서 다른 버전을 사용할 경우
   > 예시
   >
   > ```yaml
   > jobs:
