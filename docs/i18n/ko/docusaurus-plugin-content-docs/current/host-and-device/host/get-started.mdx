...
- Diagnose packages
  - 정상 Dost 실행을 위해 필요한 패키지들의 설치상태를 확인합니다.
- Open Docs
  - Dost에 대한 문서를 브라우저로 엽니다.
...
- dmg file을 열어 Dost를 Applications 폴더로 이동해주세요.
...
- WebDriverAgent
  ...
  - 이후 경고메시지나 에러메시지가 사라졌다면 Dost로 돌아와 `Click here to build` 버튼을 눌러주세요.
...
- iOSDeviceAgent
  ...
  - DoguDev 뿐안 아니라 DoguDevRunner, DoguRunner, DoguScreen에도 동일한 작업을 진행해주세요.
  - Dost로 돌아와 `Click here to build` 버튼을 눌러주세요.
  - 빌드가 완료되었다면 `Check` 버튼을 눌러 초록색 체크표시가 나오도록 해주세요.
