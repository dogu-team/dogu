...
export class AndroidResetService {
  ...
  async makeDirty(): Promise<void> {
    ...
    AndroidResetService.map.delete(this.serial);
  }
  ...
  async reset(info: DeviceSystemInfo, appiumAdb: AppiumAdb, appiumContext: AppiumContextImpl): Promise<void> {
    ...
    await retry(
      async (): Promise<void> => {
        ...
        await this.runResetWithReboot(info, appiumAdb, appiumContext);
  ...
  private async runResetWithReboot(info: DeviceSystemInfo, appiumAdb: AppiumAdb, appiumContext: AppiumContextImpl): Promise<void> {
    const { serial, logger } = this;
  ...
  private async resetDirty(): Promise<void> {
    ...
    AndroidResetService.map.set(this.serial, { lastResetTime: Date.now() });
  }
