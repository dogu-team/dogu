...
export class SeleniumDeviceWebDriverHandler implements DeviceWebDriverHandler {
  ...
  async onRelayHttp(headers: HeaderRecord, request: RelayRequest): Promise<RelayResponse> {
    ...
    this.logger.info('SeleniumDeviceWebDriverHandler.onRelayHttp', { url, method: request.method });
    let response = await httpRequestRelayHandler(url, request, this.logger);
    this.logger.info('SeleniumDeviceWebDriverHandler.onRelayHttp', { url, status: response.status });
