...
import { ExternalCommandKey } from '../../shares/external';
import { ipc } from '../../utils/window';
import styled from 'styled-components';

export type ExternalCommandKeyAndName = { key: ExternalCommandKey; name: string };
...
const ExternalCommandInstaller = ({ externalKeyAndNames, onStart, onFinish }: Props) => {
  ...
  useEffect(() => {
    ...
    const install = async () => {
      ...
      for (const { key, name } of externalKeyAndNames) {
        try {
          await ipc.externalClient.runCommand(key);
        } catch (e) {
          ipc.rendererLogger.error(`Error occurred while run: ${key} | ${stringify(e)}`);
          toast({
            title: `Error occurred while run: ${name}`,
...
export default ExternalCommandInstaller;
