import { AppConfigKey, AppConfigService as Impl, AppConfigServiceFactory } from '@dogu-private/dogu-agent-core';
...
export class AppConfigService {
  ...
  static async open(): Promise<void> {
    const impl = await new AppConfigServiceFactory().create({
      appName: app.name,
    ...
    ipcMain.handle(appConfigClientKey.getOrDefault, (_, key: AppConfigKey, value: unknown) => instance.getOrDefault(key, value));
  ...
  async getOrDefault<T = any>(key: AppConfigKey, value: T): Promise<T> {
    return this.impl.getOrDefault(key, value);
  }
