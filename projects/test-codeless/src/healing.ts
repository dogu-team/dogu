...
(async () => {
  ...
  const url = 'https://docs.bolta.io/get-started/join-bolta';
  ...
  const width = pageRect['result']['cssContentSize']['width'];
  const height = pageRect['result']['cssContentSize']['height'];
  ...
  for (let currentHeight = 0; currentHeight < height; currentHeight += 4096) {
    ...
    await driver.executeScript(`window.scrollTo(0, ${currentHeight + nextHeight});`, '');
    await wait(1000);
  ...
  function createXPath(element: cheerio.Element) {
    ...
    while (currentElement && currentElement.type !== 'root') {
      //@ts-ignore
      let siblings = currentElement.parent.children.filter(
        //@ts-ignore
    ...
    if (node.type === 'text') {
      ...
      if (textNode.data) {
        ...
        if (text.length !== 0) {
          ...
          //   path.join(
          //     __dirname,
          //     `screenshot_${boundingRect["x"]}_${boundingRect["y"]}.jpeg`
          //   ),
          //   base64["result"]["data"],
          //   "base64"
          // );
