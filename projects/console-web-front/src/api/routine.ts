...
export const readRoutine = async (organizationId: OrganizationId, projectId: ProjectId, routineId: RoutineId) => {
  const { data } = await api.get<string>(`/organizations/${organizationId}/projects/${projectId}/routines/${routineId}`);
};
...
export const createRoutine = async (organizationId: OrganizationId, projectId: ProjectId, file: File) => {
  ...
  const { data } = await api.post<RoutineBase>(`/organizations/${organizationId}/projects/${projectId}/routines`, formData, {
    headers: {
...
export const updateRoutine = async (organizationId: OrganizationId, projectId: ProjectId, routineId: RoutineId, file: File) => {
  const formData = new FormData();
  formData.append('file', file);
...
export const cancelPipeline = async (organizationId: OrganizationId, projectId: ProjectId, pipelineId: RoutinePipelineId) => {
  return await api.post<void>(`/organizations/${organizationId}/projects/${projectId}/pipelines/${pipelineId}/cancel`);
};

export const runInstantTest = async (organizationId: OrganizationId, projectId: ProjectId, dto: CreateInstantPipelineDtoBase) => {
  const { data } = await api.post<RoutinePipelineBase>(`/organizations/${organizationId}/projects/${projectId}/pipelines/instant`, dto);
