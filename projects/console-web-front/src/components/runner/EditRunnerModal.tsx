...
const EditRunnerModal = ({ isOpen, device, close }: Props) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (e instanceof AxiosError) {
        sendErrorNotification(t('runner:runnerEditFailureMsg', { message: getErrorMessage(e) }));
      }
    }
    setLoading(false);
  };
  ...
  return (
    <Modal open={isOpen} centered okText={t('common:save')} cancelText={t('common:cancel')} onCancel={close} confirmLoading={loading} onOk={handleSave} destroyOnClose>
      <H5>{t('runner:runnerEditModalTitle')}</H5>
      <FormContainer>
        <Form layout="vertical" form={form}>
          <Form.Item
            label={t('runner:runnerEditNameLabelText')}
            name="name"
            required
