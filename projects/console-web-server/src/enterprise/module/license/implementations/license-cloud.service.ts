import { CreateLicenseDto, FindLicenseWithCloudDto, LicenseBase, LicenseId, LicenseValidateClass } from '@dogu-private/console';
...
export class LicenseCloudService extends FeatureLicenseService {
  ...
  async setLicense(manager: EntityManager, dto: FindLicenseWithCloudDto): Promise<LicenseBase> {
    throw new HttpException('Method not implemented.', HttpStatus.NOT_IMPLEMENTED);
    const { licenseToken, organizationId } = dto;
  ...
  async renewLicense(manager: EntityManager, dto: FindLicenseWithCloudDto): Promise<LicenseBase> {
    throw new HttpException('Method not implemented.', HttpStatus.NOT_IMPLEMENTED);
