...
namespace mywebrtc
{
    ...
    public:
            ...
            const webrtc::CodecSpecificInfo *codec_specific_info) override
        {
            ...
            return webrtc::EncodedImageCallback::Result(
                webrtc::EncodedImageCallback::Result::OK, callback_count_);
        }
    ...
    protected:
                             ...
                             std::unique_ptr<webrtc::DesktopFrame> frame) override
        {
            ...
            output_frame_ = std::make_unique<webrtc::BasicDesktopFrame>(
                webrtc::DesktopSize(g_width, g_height));

            libyuv::ARGBScale(
                frame->data(), frame->stride(), frame->size().width(),
                frame->size().height(),
                output_frame_->GetFrameDataAtPos(webrtc::DesktopVector(0, 0)),
                output_frame_->stride(), output_frame_->size().width(),
                output_frame_->size().height(), libyuv::kFilterBilinear);

            if (!i420_buffer_.get() ||
                i420_buffer_->width() * i420_buffer_->height() < g_width * g_height)
            {
                i420_buffer_ = webrtc::I420Buffer::Create(g_width, g_height);
            }

            const int convertResult = libyuv::ConvertToI420(
                output_frame_->data(), 0, i420_buffer_->MutableDataY(),
                i420_buffer_->StrideY(), i420_buffer_->MutableDataU(),
                i420_buffer_->StrideU(), i420_buffer_->MutableDataV(),
                i420_buffer_->StrideV(), 0, 0, g_width, g_height, g_width, g_height,
    ...
    void createEncoder()
    {
        ...
        g_encoder = std::make_unique<webrtc::LibvpxVp8Encoder>(
            std::move(vpxInterface), webrtc::VP8Encoder::Settings());
        ...
        codec_settings_.SetVideoEncoderComplexity(
            webrtc::VideoCodecComplexity::kComplexityNormal);
    ...
    void startCapture()
    {
        ...
        std::chrono::time_point<std::chrono::system_clock> periodStartTime =
            std::chrono::system_clock::now();
        while (true)
        {
            ...
            auto frameElapsedMilisec =
                std::chrono::duration_cast<std::chrono::milliseconds>(frameElapsedSec);
            ...
            if (1 < remainFrameDeltaMillisec)
            {
                std::this_thread::sleep_for(
                    std::chrono::milliseconds(remainFrameDeltaMillisec - 1));
            }
