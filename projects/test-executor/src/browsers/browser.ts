...
export abstract class BrowserDriver {
  ...
  protected createUserAgent(): string {
    switch (this.device.vendor) {
      case 'Samsung':
        return `Mozilla/5.0 (Linux; Android 13) AppleWebKit/604.1 (KHTML, like Gecko) Chrome/118.0.0.0 Mobile Safari/604.1`;
      case 'Apple':
      ...
      default:
        return `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/604.1 (KHTML, like Gecko) Chrome/118.0.0.0 Safari/604.1`;
    }
  }

  protected createXPath(node: cheerio.ParentNode | cheerio.Node) {
  ...
  protected async setElementOpacity(node: cheerio.ParentNode | cheerio.Node, opacity: number) {
