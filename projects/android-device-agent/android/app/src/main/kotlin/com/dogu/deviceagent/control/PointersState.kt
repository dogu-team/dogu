...
class PointersState {
    private val pointers: MutableList<Pointer> = ArrayList<Pointer>()
    private fun indexOf(id: Long): Int {
        for (i in pointers.indices) {
            val pointer: Pointer = pointers[i]
            if (pointer.id === id) {
                return i
            }
        }
    ...
    fun getPointerIndex(id: Long): Int {
        ...
        if (localId == -1) {
            throw InternalException("pointers.size() < maxFingers implies that a local id is available")
        }
