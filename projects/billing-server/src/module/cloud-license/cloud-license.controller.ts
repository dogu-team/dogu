...
export class CloudLicenseController {
  ...
  async createCloudLicense(@Body() dto: CreateCloudLicenseDto): Promise<CloudLicense> {
    return await this.cloudLicenseService.createOrUpdateCloudLicense(dto);
  }

  @Put()
  @BillingTokenPermission()
  async createOrUpdateCloudLicense(@Body() dto: CreateCloudLicenseDto): Promise<CloudLicense> {
    return await this.cloudLicenseService.createOrUpdateCloudLicense(dto);
  }
