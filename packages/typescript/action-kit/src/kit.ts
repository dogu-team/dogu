...
import { DeviceServerToken, OrganizationId, ProjectId } from '@dogu-tech/types';
...
export class ActionKit {
  ...
  run(onRun: (context: ActionContext) => PromiseOrValue<void>): void {
    (async () => {
      ...
      const deviceClients = this.createDeviceClients(ActionLogger, DOGU_DEVICE_SERVER_PORT, DOGU_DEVICE_TOKEN, DOGU_REQUEST_TIMEOUT);
  ...
  private createDeviceClients(printable: FilledPrintable, DOGU_DEVICE_SERVER_PORT: string, DOGU_DEVICE_TOKEN: string, DOGU_REQUEST_TIMEOUT: number): DeviceClients {
