...
import { CUSTOM_CHILD, DIVIDER, generateSubtag, H1, H2, H3, H4, H5, UL, UL2 } from './markdown-tag';
...
export class CodeGenTypescript {
  ...
  private static async parseCodeFile(filePath: string, option: Partial<TypescriptGenerateOption>): Promise<CodeGenElems> {
    const parser = new TypescriptParser();
    const parsed = await parser.parseFile(filePath, workspacePath);
