...
export class LinuxChannel implements DeviceChannel {
  ...
  static async create(param: DeviceChannelOpenParam, deviceServerService: DeviceServerService): Promise<DeviceChannel> {
    ...
    const streaming = await PionStreamingService.create(param.serial, Platform.PLATFORM_LINUX, env.DOGU_DEVICE_SERVER_PORT, createGdcLogger(param.serial));
    ...
    const deviceChannel = new LinuxChannel(param.serial, info, new DesktopProfileService(param.serial, logger), streaming, deviceAgent, seleniumDeviceWebDriverHandler, []);
