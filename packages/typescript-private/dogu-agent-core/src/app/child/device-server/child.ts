...
export class DeviceServerChild implements Child {
  ...
  async open(): Promise<void> {
    ...
    const DOGU_IS_DEVICE_SHAREABLE = appConfigService.getOrDefault<boolean>('DOGU_IS_DEVICE_SHAREABLE', false);
    ...
    const options = await fillChildOptions({
      forkOptions: {
        env: {
          ...
          DOGU_IS_DEVICE_SHAREABLE: DOGU_IS_DEVICE_SHAREABLE ? 'true' : 'false',
